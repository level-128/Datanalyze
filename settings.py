...
import wx
from config.config_library import save_config, printf, debug_log, config, reset_config
...
class Storm(wx.Frame):
	...
	printf(f"the settings opened with {dpi_scale=} on {__name__}")
	...
	def set_panel(self):
		...
		wx.StaticText(self.panel, pos=self.x(10, self.get_next_poz(0) + 3), label="This is the setting utility of the Datanalyze. \n"
			"Change the settings are NOT recommended, \nunless for advanced users.").SetFont(self.large_winfont)
		
		reset_btn = wx.Button(self.panel, pos=self.x(40, self.get_next_poz(CONST_LARGE_GAP) + 5), label="RESET ALL SETTINGS AND EXIT.", size=(300,
		                                                                                                                                      40))
		...
		def get_checkbox(content: str, constant: int = CONST_SMALL_GAP, /, font: wx.Font = self.winfont):
			_ = wx.CheckBox(self.panel, pos=self.x(5 + self.ui_xpoz_plus, self.get_next_poz(constant)), label=content, size=self.x(300, 35))
			_.SetFont(font)
			return _

		def get_static_text(content: str, constant: int = CONST_SMALL_GAP, /, font: wx.Font = self.winfont):
			wx.StaticText(self.panel, pos=self.x(5 + self.ui_xpoz_plus, _ := self.get_next_poz(constant)), label=content).SetFont(font)
			return _

		get_static_text("1. figure settings: ", CONST_LARGE_GAP, font=self.title_font)
		...
		get_static_text("2. display settings: ", CONST_LARGE_GAP, font=self.title_font)
		...
		get_static_text("3.Compute settings", CONST_LARGE_GAP, font=self.title_font)
		...
		get_static_text('Enable Fast Math will shorten the compute cost, causing \nunreliable results. \nNOTE: THIS FEATURE'
		...
		get_static_text("selecting float number type. Enable float32 will decrease \nthe accuracy of the result but requires less time\n to "
		                "compute.", CONST_MID_GAP)
		
		self.input_is_float32= get_checkbox("Use float32", CONST_MID_GAP)

		get_static_text(
			'Perform AOT parallel in each computation. \nNOTE: THIS OPTION IS NOT RECOMMENDED, FORCE \nENABLE AOT PARALLEL WILL SPEND MORE TIME BEFORE\n'
		...
		get_static_text("4. Other settings", CONST_LARGE_GAP, font=self.title_font)
	...
	def save_settings_config(self):
		for element in self.panel_elements:
			...
			if type(raw_element) is bool:
				config["$default_" + element[6:]] = raw_element

			elif raw_element.find('None') != -1:
				config["$default_" + element[6:]] = None

			elif not (not raw_element.find('.') + 1 or not (_ := raw_element.replace('.', '', 1).isdigit() or (not (not (_[0] == '-') or not _[1:].isdigit())))):
				config["$default_" + element[6:]] = float(raw_element)
				
			elif raw_element.isdigit() or (not (not (raw_element[0] == '-') or not raw_element[1:].isdigit())):
				config["$default_" + element[6:]] = int(raw_element)
	...
	def on_reset(self, *void):
		dlg = wx.MessageDialog(self.panel, "Do You Want to reset all settings?", "Reset", style=wx.YES_NO | wx.NO_DEFAULT)
		if dlg.ShowModal() == wx.ID_YES:
			reset_config()
	
	@debug_log
	def on_close(self, *void) -> None:
		dlg = wx.MessageDialog(self.panel, "Do You Want to save before exit?", "Exit", style=wx.YES_NO | wx.CANCEL | wx.CANCEL_DEFAULT)
