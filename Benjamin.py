...
and calling the plotting api in the Catherine.py
...
def plot(input_str: str, python_mode: bool = False, is_new_fig=False):
    ...
    if current_plt is None or is_new_fig:
        current_plt = Catherine.Christopher()
    
    if python_mode:
        exec(input_str)
    else:
        interpret_to_falcon(input_str)
...
@debug_log
def interpret_to_falcon(input_str: str) -> None:
    """
